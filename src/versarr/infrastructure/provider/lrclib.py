...
from versarr.observability import MetricsFacade
...
class LrclibProvider(LyricsProvider):
    ...
    async def fetch(self, identity: TrackIdentity) -> ProviderResult:
        ...
        try:
            ...
            self._metrics.provider_requests_total.labels(provider="lrclib", status="transient_failure").inc()
        ...
        if response.status_code == 404:
            self._metrics.provider_requests_total.labels(provider="lrclib", status="not_found").inc()
            return ProviderResult(
        ...
        if not lyrics_text:
            self._metrics.provider_requests_total.labels(provider="lrclib", status="invalid_content").inc()
            return ProviderResult(
